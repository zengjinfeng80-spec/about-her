...
当前版本已在客户端暂停内置 AI：新记录只保存和同步原始文字、图片与录音，不创建分析任务，也不显示“问记录”、分析状态或重试入口。保存包含文字的新记录时，必须选择一个档案分类，系统会用整条原文自动加入已确认档案；只有图片或语音的记录只保存原件、不自动入档。旧记录仍可通过“加入档案”手动补录；以上过程不会调用模型。服务端 AI 结构仅为后续开发保留，当前未配置模型密钥。
